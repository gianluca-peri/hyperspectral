...
def get_all_runs(base_dir="cifar100_vit"):
    runs = []
...
def aggregate_data(runs):
    ...
    aggregated = {
        "standard_vit": {"train_loss": [], "val_loss": [], "val_accuracy": []},
        "spectral_triadic_vit": {"train_loss": [], "val_loss": [], "val_accuracy": []}
    }
    
    for run_dir in runs:
        std_path = os.path.join(run_dir, "standard_vit", "history.json")
        spectral_path = os.path.join(run_dir, "spectral_triadic_vit", "history.json")
        
        if os.path.exists(std_path):
            data = load_history(std_path)
            if "train_loss" in data:
                aggregated["standard_vit"]["train_loss"].append(data["train_loss"])
            if "val_loss" in data:
                aggregated["standard_vit"]["val_loss"].append(data["val_loss"])
            if "val_accuracy" in data:
                aggregated["standard_vit"]["val_accuracy"].append(data["val_accuracy"])
            
        if os.path.exists(spectral_path):
            data = load_history(spectral_path)
            if "train_loss" in data:
                aggregated["spectral_triadic_vit"]["train_loss"].append(data["train_loss"])
            if "val_loss" in data:
                aggregated["spectral_triadic_vit"]["val_loss"].append(data["val_loss"])
            if "val_accuracy" in data:
                aggregated["spectral_triadic_vit"]["val_accuracy"].append(data["val_accuracy"])
...
def main():
    ...
    # Plot Train Loss
    plot_metric(ax1, data["standard_vit"], "train_loss", "Standard ViT", "o", c_std)
    plot_metric(ax1, data["spectral_triadic_vit"], "train_loss", "Spectral Triadic ViT", "^", c_spec)
    ...
    plot_metric(ax2, data["standard_vit"], "val_accuracy", "Standard ViT", "o", c_std)
    plot_metric(ax2, data["spectral_triadic_vit"], "val_accuracy", "Spectral Triadic ViT", "^", c_spec)
    ...
    max_std = max_val_accuracy(data["standard_vit"])
    max_spec = max_val_accuracy(data["spectral_triadic_vit"])
    ...
    plot_metric(ax3, data["standard_vit"], "val_loss", "Standard ViT", "o", c_std)
    plot_metric(ax3, data["spectral_triadic_vit"], "val_loss", "Spectral Triadic ViT", "^", c_spec)
    ...
    # Save plot
    output_path = os.path.join(base_dir, "graph_cifar100vit.png")
    plt.tight_layout()
