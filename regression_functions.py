...
import torch
import sympy as sp
from list_of_functions import x, y
import os
from utlilities import save_dir_function

def generate_dataloaders(func, dim=1, num_samples=1000, batch_size=32, noise_std=0.0):
    # Generate random input data in range [-1, 1]
    X = np.random.uniform(-1, 1, (num_samples, dim)).astype(np.float32)  # Shape (num_samples, dim)
...
def train_and_evaluate(model, train, test, epochs=100, learning_rate=1e-3, **kwargs):
